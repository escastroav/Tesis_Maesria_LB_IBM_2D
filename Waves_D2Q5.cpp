...
double LatticeBoltzmann::Speed_Capsule(int ix, int iy, int X, int Y, double R, double v,double Phi)
{
  ...
  double Rp = 2*R / M_PI;
  double r1 = (xr-R)*(xr-R) + yr*yr;
  double r2 = (xr+R)*(xr+R) + yr*yr;
  double w = 1.0/3.0;
  double circle1 = 1.0 - tanh(r1-Rp*Rp);
  double circle2 = 1.0 - tanh(r2-Rp*Rp);
  double circles = circle1 + circle2 - 0.5*circle1*circle2;
  double slab_X = tanh((xr+R)/w) - tanh((xr-R)/w);
  double slab_Y = tanh((yr+Rp)/w) - tanh((yr-Rp)/w);
  double slab = 0.5*slab_X*slab_Y;
